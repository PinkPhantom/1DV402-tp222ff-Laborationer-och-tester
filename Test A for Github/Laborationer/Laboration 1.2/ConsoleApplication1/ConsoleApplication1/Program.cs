...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        private static void Answer(double total, int topay)
        {
            ...
            int[] change = new int[7] { 500, 100, 50, 20, 10, 5, 1 };



            ArrayForChange(ref back, change);
            return;
        ...
        private static void ArrayForChange(ref int back, int[] change)
        {
            change[1] = back / change[1];
            back %= change[1];
            Console.WriteLine("500-lappar : {0}st ", change[1]);

            change[2] = back / change[2];
            back %= change[2];
            Console.WriteLine("100-lappar : {0}st ", change[2]);

            change[3] = back / change[3];
            back %= change[3];
            Console.WriteLine("50-lappar : {0}st ", change[3]);

            change[4] = back / change[4];
            back %= change[4];
            Console.WriteLine("20-lappar : {0}st ", change[4]);

            change[5] = back / change[5];
            back %= change[5];
            Console.WriteLine("10-kronor : {0}st ", change[5]);

            change[5] = back / change[5];
            back %= change[5];
            Console.WriteLine("5-kronor : {0}st ", change[5]);

            change[5] = back / change[1];
            back %= change[5];
            Console.WriteLine("1-kronor : {0}st ", change[5]);
